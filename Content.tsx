...
export function Content() {
  ...
  const isBoxVisibleType = detectType === '2D bounding boxes' || detectType === 'Price prediction' || detectType === 'Text extraction';
          ...
          boundingBoxes2D.map((box, i) => (
            <div
              ...
              style={{
                ...
                borderColor: detectType === 'Text extraction' ? '#10b981' : '#3B68FF',
              }}>
              <div 
                className={`text-white absolute left-0 top-0 text-sm px-1 flex flex-col items-start ${box.price ? 'gap-0' : ''}`}
                style={{backgroundColor: detectType === 'Text extraction' ? '#10b981' : '#3B68FF'}}
              >
                <span className="font-bold">{box.label}</span>
                {box.price && <span className="text-[10px] bg-yellow-400 text-black px-1 rounded-sm">💰 {box.price}</span>}
              </div>
            </div>
          ))}
