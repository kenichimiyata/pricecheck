...
export function Prompt() {
  ...
  const is2d = detectType === '2D bounding boxes' || detectType === 'Price prediction' || detectType === 'Text extraction';

  const currentModel = selectedModel;

  const get2dPrompt = () => {
    if (detectType === 'Price prediction') {
      return `Detect ${targetPrompt}, estimate their current market price in Japanese Yen (JPY), and output as a JSON list where each entry contains "box_2d", "label", and "price" (string with ¥). Limit to 20 items.`;
    }
    if (detectType === 'Text extraction') {
      return `Detect and extract all visible ${targetPrompt} from the image. Output a JSON list where each entry contains "box_2d" and "label" (the transcribed text). Limit to 30 items.`;
    }
    return `Detect ${targetPrompt}, with no more than 20 items. Output a json list where each entry contains the 2D bounding box in "box_2d" and a text label in "label".`;
  };
  ...
  async function handleSend() {
    ...
    try {
      ...
      if (detectType === '2D bounding boxes' || detectType === 'Price prediction' || detectType === 'Text extraction') {
        const formattedBoxes = parsedResponse.map(
          (box: {box_2d: [number, number, number, number]; label: string; price?: string}) => {
      ...
      <div className="w-full flex flex-col">
        {is2d ? (
          <div className="flex flex-col gap-2">
            <div>{detectType === 'Text extraction' ? 'Extract text from:' : 'Detect items:'}</div>
            <textarea
              className="w-full bg-[var(--input-color)] rounded-lg resize-none p-4"
              placeholder={detectType === 'Text extraction' ? "e.g., labels, signs, text" : "e.g., cars, trees"}
              rows={1}
              value={targetPrompt}
